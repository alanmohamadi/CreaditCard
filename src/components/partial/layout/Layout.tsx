import type { ReactNode } from "react";
import { useLocation, Navigate } from "react-router";
import Login from "../../../page/login";
import { useAuth } from "../../../core/provider/AuthContext";

export default function Layout({ children }: { children: ReactNode }) {
    const location = useLocation();
    const { isAuthenticated } = useAuth();

    if (location.pathname === "/login") {
        if (isAuthenticated) {
            return <Navigate to="/" replace />;
        }
        return (
            <div className="w-full max-w-[400px]  h-dvh bg-black overflow-hidden flex flex-col mx-auto md:border rounded-2xl">
                <header className="h-10 text-white bg-black flex items-center justify-between px-7 z-10">
                    <p>9:41</p>
                    <div className="flex gap-1 items-center justify-center">
                        <div className="flex gap-1 items-center justify-center">
                            <svg width="78" height="13" viewBox="0 0 78 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path opacity="0.35" d="M54 0.527344H71C72.9178 0.527344 74.4727 2.08222 74.4727 4V9C74.4727 10.9178 72.9178 12.4727 71 12.4727H54C52.0822 12.4727 50.5273 10.9178 50.5273 9V4C50.5273 2.08222 52.0822 0.527344 54 0.527344Z" stroke="#EAEAEA" stroke-width="1.05509" />
                                <path opacity="0.4" d="M76 5V9.22034C76.8491 8.86291 77.4012 8.0314 77.4012 7.11017C77.4012 6.18894 76.8491 5.35744 76 5Z" fill="#EAEAEA" />                                 <path d="M52 4C52 2.89543 52.8954 2 54 2H71C72.1046 2 73 2.89543 73 4V9C73 10.1046 72.1046 11 71 11H54C52.8954 11 52 10.1046 52 9V4Z" fill="#EAEAEA" />
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M34.5005 3.58753C36.967 3.58764 39.3393 4.55505 41.1269 6.28982C41.2615 6.42375 41.4766 6.42206 41.6092 6.28603L42.896 4.96045C42.9631 4.89146 43.0006 4.798 43 4.70076C42.9994 4.60353 42.9609 4.51052 42.893 4.44234C38.2011 -0.147446 30.7991 -0.147446 26.1072 4.44234C26.0392 4.51047 26.0006 4.60345 26 4.70069C25.9994 4.79792 26.0367 4.89141 26.1038 4.96045L27.391 6.28603C27.5235 6.42226 27.7388 6.42396 27.8733 6.28982C29.6612 4.55494 32.0337 3.58752 34.5005 3.58753ZM34.5359 7.67235C35.8911 7.67227 37.198 8.18644 38.2025 9.11497C38.3384 9.24674 38.5524 9.24389 38.6849 9.10853L39.9702 7.78295C40.0379 7.71342 40.0754 7.61909 40.0744 7.52108C40.0735 7.42306 40.034 7.32954 39.965 7.26142C36.9059 4.35683 32.1685 4.35683 29.1094 7.26142C29.0403 7.32953 29.0009 7.42311 29 7.52115C28.9991 7.6192 29.0368 7.71352 29.1046 7.78295L30.3895 9.10853C30.522 9.24389 30.736 9.24674 30.8719 9.11497C31.8758 8.18706 33.1816 7.67293 34.5359 7.67235ZM37.1496 10.3427C37.1515 10.441 37.1137 10.5357 37.0449 10.6046L34.8217 12.8948C34.7565 12.9621 34.6676 13 34.5749 13C34.4822 13 34.3933 12.9621 34.3282 12.8948L32.1045 10.6046C32.0358 10.5357 31.998 10.4409 32.0001 10.3426C32.0021 10.2443 32.0438 10.1512 32.1153 10.0853C33.5351 8.8595 35.6147 8.8595 37.0345 10.0853C37.106 10.1513 37.1476 10.2444 37.1496 10.3427Z" fill="#EAEAEA" />
                                <path d="M10 4C10 3.44772 10.4477 3 11 3H12C12.5523 3 13 3.44772 13 4V12C13 12.5523 12.5523 13 12 13H11C10.4477 13 10 12.5523 10 12V4Z" fill="#EAEAEA" />
                                <path d="M15 2C15 1.44772 15.4477 1 16 1H17C17.5523 1 18 1.44772 18 2V12C18 12.5523 17.5523 13 17 13H16C15.4477 13 15 12.5523 15 12V2Z" fill="#EAEAEA" />
                                <path d="M5 7.5C5 6.94772 5.44772 6.5 6 6.5H7C7.55228 6.5 8 6.94772 8 7.5V12C8 12.5523 7.55228 13 7 13H6C5.44772 13 5 12.5523 5 12V7.5Z" fill="#EAEAEA" />
                                <path d="M0 10C0 9.44772 0.447715 9 1 9H2C2.55228 9 3 9.44772 3 10V12C3 12.5523 2.55228 13 2 13H1C0.447715 13 0 12.5523 0 12V10Z" fill="#EAEAEA" />
                            </svg>

                        </div>
                    </div>
                </header>

                <main className="flex-1 min-h-0 rounded-t-2xl overflow-y-auto no-scrollbar scroll-smooth">
                    <div className="h-full p-2 pb-4">
                        <Login />
                    </div>
                </main>
                <footer className="h-1.25 w-42.5 mx-auto rounded-full  bg-gray-200 flex items-center justify-center">
                </footer>
            </div>
        );
    }

    if (!isAuthenticated) {
        return <Navigate to="/login" state={{ from: location }} replace />;
    }

    return (
        <div className="w-full max-w-[400px] h-dvh bg-black overflow-hidden flex flex-col mx-auto md:border rounded-2xl">
            <header className="h-10 text-white bg-black flex items-center justify-between px-7 z-10">
                <p>9:41</p>
                <div className="flex gap-1 items-center justify-center">

                </div>
            </header>

            <main className="flex-1 min-h-0 rounded-t-2xl overflow-y-auto no-scrollbar scroll-smooth">
                <div className="h-full p-2 pb-4">
                    {children}
                </div>
            </main>

            <footer className="h-1.25 w-42.5 mx-auto rounded-full bg-gray-200 flex items-center justify-center">
            </footer>
        </div>
    );
}
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      <form onSubmit={handleSubmit} className="flex flex-col">
        <button
          type="button"
          onClick={onBack}
          className="w-9 h-9 rounded-full bg-[#242424] flex items-center justify-center text-white mb-5"
        >
          <ArrowLeft size={18} />
        </button>

        <h2 className="text-2xl font-bold text-white mb-1">Add new card</h2>
        <p className="text-sm text-[#EAEAEA99] leading-5 mb-6">
          We recommend you to set card for further reference too you can use it anytime
        </p>

        <label className="text-sm font-semibold text-white mb-2">Card number</label>
        <div
        ...
        <div className="flex items-center justify-between mt-6">
          <span className="text-sm text-[#EAEAEA99]">Save this card for future refrence</span>
          <button
            type="button"
            onClick={() => setSaveCard((s) => !s)}
            className={`w-11 h-6 rounded-full flex items-center px-0.5 transition-colors ${
              saveCard ? "bg-[#00B8F0] justify-end" : "bg-[#3a3a3a] justify-start"
            }`}
          >

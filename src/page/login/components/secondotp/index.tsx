...
const SecondOTP: React.FC<SecondOTPProps> = ({ onSuccess, goToStep }) => {
  ...
  const formik = useFormik<FormValues>({
    ...
    onSubmit: (values: FormValues) => {
      ...
      if (otp === '5678') {
        ...
        formik.setErrors({ otp: 'Invalid PIN' });
      }
    },
  });
  ...
  const handleChange = (index: number, value: string) => {
    ...
    if (newOtp.every(digit => digit !== '')) {
      ...
      if (fullOtp === '5678') {
        localStorage.setItem('finalOTP', fullOtp);
        onSuccess();
      } else {
        setError('Invalid PIN. Please try again.');
        formik.setErrors({ otp: 'Invalid PIN' });
  ...
  return (
    <div className="w-full bg-[000000] px-3">
      <div className='w-full flex justify-between items-center'>
        <div className="gap-2 cursor-pointer" onClick={handleArrowClick}>
      ...
      <div className="mb-8">
        <h2 className="text-4xl font-bold text-white mb-2 mt-7">Create 4 digit pin</h2>
        <p className="text-md text-[#EAEAEA99] leading-relaxed">
          We recommend you to set 4 digit pin for secure and safe mobile transaction
        </p>
      </div>
        ...
        <div className="flex gap-3 text-amber-50 mb-2">
          {[0, 1, 2, 3].map((index) => (
            <input
              ...
              className={`w-14 h-14 text-center border border-[#2c2b2b] text-2xl bg-[#2c2b2b] font-semibold rounded-xl outline-none transition-colors ${
